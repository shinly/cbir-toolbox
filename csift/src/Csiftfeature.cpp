				   ...
				   int img_dbl, int descr_width, int descr_hist_bins )
{
	...
	// DWORD dw_pyr = GetTickCount();
	// DWORD time_pyr = dw_pyr - dwStart;

	dog_pyr = build_dog_pyr( gauss_pyr, octvs, intvls );

	// DWORD dw_dog = GetTickCount();
	// DWORD time_dog = dw_dog - dw_pyr;

	storage = cvCreateMemStorage( 0 );
	features = scale_space_extrema( dog_pyr, octvs, intvls, contr_thr,
		curv_thr, storage );

	// DWORD dw_extrema = GetTickCount();
	// DWORD time_extrema = dw_extrema - dw_dog;
	...
	// DWORD dw_os = GetTickCount();
	// DWORD time_os = dw_os - dw_extrema;

	compute_descriptors( features, gauss_pyr, descr_width, descr_hist_bins );

	// DWORD dw_des = GetTickCount();
	// DWORD time_des = dw_des - dw_os;
...
void Csiftfeature::_extractSIFT(){

	SIFTnum= _sift_features( img, &m_features, intvls, sigma, contr_thr, curv_thr,
						img_dbl, descr_width, descr_hist_bins );

}
int Csiftfeature::_getSIFTnum(){
	
	return SIFTnum;
}

void Csiftfeature::_copySIFTarrary(simpfeature* newfeature){
	for(int j=0; j< SIFTnum; j++){
		newfeature[j].x = m_features[j].x;
		newfeature[j].a = m_features[j].a;
		newfeature[j].b = m_features[j].b;
		newfeature[j].c = m_features[j].c;
		newfeature[j].scl = m_features[j].scl;
		newfeature[j].ori = m_features[j].ori;
		newfeature[j].d = m_features[j].d;
		for (int i= 0;i<128; i++){
			newfeature[j].descr[i] = m_features[j].descr[i];
		}
		newfeature[j].type = m_features[j].type;
		newfeature[j].category = m_features[j].category;
		newfeature[j].img_pt = m_features[j].img_pt;
		newfeature[j].mdl_pt = m_features[j].img_pt;
	}

}
void Csiftfeature::_saveSIFT_Text(string saveFile)
{
		// save feature vector into a file
	ofstream outFile;
	outFile.open(saveFile.c_str());

	if (!outFile)
	{
		cout << "Fail to open the save file" << saveFile << endl;
		return;
	}
	else
	{
		int siftnum = _getSIFTnum();
		int saveMode = 1;
		//outFile << saveMode << endl;
		outFile << siftnum  << endl;
		
		unsigned char sift_temp;
		float para_temp;
		for (int i = 0; i < siftnum; i++)
		{		
			for (int j = 0; j < FEATURE_MAX_D; j++)
			{
				sift_temp = (unsigned char)(m_features[i].descr[j]);
				outFile << sift_temp << "\t";
			}

			para_temp =float(m_features[i].x);
			outFile << para_temp << "\t";

			para_temp =float(m_features[i].y);
			outFile << para_temp << "\t";

			para_temp =float(m_features[i].scl);
			outFile << para_temp << "\t";

			para_temp =float(m_features[i].ori);
			outFile << para_temp << "\t";
...
void Csiftfeature::_saveSIFT_Binary(string saveFile)
{
	ofstream outFile(saveFile.c_str(), ios::binary);
	if (!outFile.good())
	{
		cout << "Fail to open the save file" << saveFile << endl;
		return;
	...
	int siftnum = _getSIFTnum();
	//int saveMode = 1;
	//outFile.Write(&saveMode, sizeof(int));
	outFile.write((char *)&siftnum, sizeof(int));
	unsigned char sift_temp[FEATURE_MAX_D];
	float para_temp;
	for (int i = 0; i < siftnum; i++)
	{
		for (int j = 0; j < FEATURE_MAX_D; j++)
		{
			sift_temp[j] = (unsigned char)(m_features[i].descr[j]);
		}
		outFile.write((char *)sift_temp, FEATURE_MAX_D);

		para_temp =float(m_features[i].x);
		outFile.write((char *)&(para_temp), sizeof(float));
...
// void Csiftfeature::_saveSIFT_Binary(wstring saveFile)
// {
// 	// save feature vector into a file
// 	CFile outFile;
// 	if (!(outFile.Open(saveFile.c_str(), CFile::modeCreate | CFile::modeWrite | CFile::typeBinary)))
// 	{
// 		wcout << "Fail to open the save file" << saveFile << endl;
// 		return;
// 	}
// 	else
// 	{
...
// 		//int saveMode = 1;
// 		//outFile.Write(&saveMode, sizeof(int));
// 		outFile.Write(&siftnum, sizeof(int));
// 		unsigned char sift_temp[FEATURE_MAX_D];
// 		float para_temp;
// 		for (int i = 0; i < siftnum; i++)
// 		{
...
// 				sift_temp[j] = unsigned char(m_features[i].descr[j]);
// 			}
// 			outFile.Write(sift_temp, FEATURE_MAX_D);

// 			para_temp =float(m_features[i].x);
// 			outFile.Write(&(para_temp), sizeof(float));
...
void Csiftfeature::_initialize(IplImage* in_image){
	//img = in_image;

	img = cvCreateImage(cvGetSize(in_image), in_image->depth, in_image->nChannels);
	cvCopy(in_image, img, NULL);
}

void Csiftfeature::_drawfeatures(){

	img_draw=img;
	draw_features( img_draw, m_features, SIFTnum );
}

void Csiftfeature::_showdrawnimg(){
	cvNamedWindow( "the drawn image", 1 );
	cvShowImage( "the drawn image", img_draw );
	cvWaitKey( 0 );
}
...
Csiftfeature::Csiftfeature(IplImage* in_image){
...
bool IsImageFile(const wchar_t * filename)
{
		...
		for(int i = 0; filename[i+index+1] != NULL; i++)
		{
			ext[i] = filename[i+index+1];
		}
        ...
        static wchar_t * allowed_ext[]={L"jpg", L"bmp", L"jpeg", L"gif",L"tif",L"tiff",L"png"};
        static int size = sizeof(allowed_ext)/sizeof(allowed_ext[0]);

        for( int i = 0; i < size; ++i )
              if( wcscmp(ext, allowed_ext[i]) == 0 )
                  return true;
...
// 			wstring dir_child = m_SrcFolder + L"\\" + ffd.cFileName;
			
// 			if ((ffd.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY)) // 如果是一个目录的话
// 			{
// 				// 处理该目录下的所有的图像
// 				ExtractFolderFeatures(dir_child, img_num);								
// 			}
// 			else if( IsImageFile(ffd.cFileName) )
// 			{
// 				wstring sift_saveFile = dir_child + L".sift";
// 				if (_waccess(sift_saveFile.c_str(), 0) != 0)  // if the sift file does not exist
// 				{
// 					wofstream outFile;
// 					outFile.open(sift_saveFile.c_str(), wofstream::out);
// 					outFile.close();
// 					ExtractSiftFeature(dir_child, sift_saveFile, img_num);
// 				}
// 			}
// 		} // while		
// 	} // if			
...
// Extract sift features for all image files in a directory
...
{	
	...
	// char pchar[MAX_PATH]={0};
	// wstring tPath = srcDir;
	// WideCharToMultiByte( CP_ACP, 0, tPath.c_str() , -1,pchar, MAX_PATH, NULL, NULL );
	IplImage *img_new;
	if( (img_new = cvLoadImage(srcDir.c_str())) == 0)
	{
		cout<< "Fail to load file " << srcDir <<endl;
		return;     
	}
	if (img_new->width < 20 || img_new->height < 20)
	{
		cout<< "The image file is not valid" << srcDir <<endl;
		cvReleaseImage(&img_new);
		return;     
	}

	cout << "Performing image " << m_imgNum << " : " << srcDir << endl;

	Csiftfeature *siftExtractor = new Csiftfeature[1];

	if (max(img_new->width, img_new->height) > 640)  // the larger axis should be smaller than 400
	{
		double scale = (double)max(img_new->width, img_new->height) / 640;
		CvSize m_size;
		m_size.width = int(img_new->width / scale); 
		m_size.height = int(img_new->height / scale); 
		
		IplImage *newimage = cvCreateImage(m_size, IPL_DEPTH_8U, img_new->nChannels); 
		cvResize(img_new, newimage, CV_INTER_AREA);  // resize image for computing convenience

		siftExtractor->_initialize(newimage);

		cvReleaseImage(&newimage);
		cvReleaseImage(&img_new);

		wcout << "Image of great size is re-scaled :"
			  << m_size.width
			  << " by "
			  << m_size.height
			  << endl;
	...
	{
		siftExtractor->_initialize(img_new);
	}
	cvReleaseImage(&img_new);

	//----------------------Extract SIFT feature
	siftExtractor->_extractSIFT();
	//siftExtractor->_drawfeatures();
	//siftExtractor->_showdrawnimg();
	int siftnum = siftExtractor->_getSIFTnum();


	//siftExtractor->_saveSIFT_Text(saveFile);
	siftExtractor->_saveSIFT_Binary(saveFile);
	
	m_imgNum ++;
	delete []siftExtractor;


	// dwStop =  GetTickCount();   
	// cout<< "Time elapse: " << dwStop - dwStart << " ms" <<endl;
}
...
{	
	...
	ofstream outFile(saveFile.c_str(), ios::out);
	if (!outFile.good())
	{
		cout << "Fail to open the save file" << saveFile << endl;
		return;
	...
	for (int i = 0; i < siftnum; i++)
	{
		outFile << siftExtractor->m_features[i].x << " " << siftExtractor->m_features[i].y << endl;
	}
	outFile.close();
